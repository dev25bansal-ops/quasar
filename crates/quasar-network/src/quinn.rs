...
pub struct QuinnBackend {
    ...
    conn_tx: mpsc::UnboundedSender<(SocketAddr, quinn::Connection)>,
...
impl QuinnBackend {
    ...
    fn build_client_config() -> ClientConfig {
        ...
        let crypto = rustls::ClientConfig::builder().with_no_client_auth();
                ...
                tokio::spawn(async move {
                    match incoming.await {
                        Ok(conn) => {
                            ...
                            Self::spawn_recv_loop(recv_conn, addr, tx);
                        }
                        Err(e) => {
            ...
            match endpoint.connect(addr, "localhost") {
                Ok(connecting) => match connecting.await {
                    Ok(conn) => {
                        ...
                        let recv_conn = conn.clone();
                        let _ = conn_tx.send((addr, conn));
                        QuinnBackend::spawn_recv_loop(recv_conn, addr, tx);
                    }
                    Err(e) => {
                        let _ = tx.send(QuicEvent::Disconnected(
                            addr,
